...
type SlideInOutOptions = Record<string, unknown>;

type NavigateWithTransition = (
  href: string,
  onRouteChange?: () => void,
  options?: SlideInOutOptions
) => void;

type ViewTransitionReturn = {
  navigateWithTransition: NavigateWithTransition;
  router: ReturnType<typeof useRouter>;
};

const createSVGOverlay = (): HTMLDivElement => {
...
export const useViewTransition = (): ViewTransitionReturn => {
  const router = useRouter();

  const navigateWithTransition: NavigateWithTransition = (
    href,
    onRouteChange,
    _options = {}
  ) => {
    ...
    slideInOut(href, onRouteChange, router);
  };
